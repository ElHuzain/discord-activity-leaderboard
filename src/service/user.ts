...
class UserService {
    ...
    async syncUsers(client: Client) {
        ...
        allVoiceChannels.forEach(async (channel: BaseGuildVoiceChannel) => {
            channel.members.forEach(async (member: GuildMember) => {
                ...
                const isRegistered = registeredUsers.find(user => user.id === member.id);

                if (!isRegistered) {
                    const userFromRepo = await this.UserRepository.getById(member.id);

                    if(userFromRepo) {
                        userFromRepo.lastJoinedAt = Date.now();
    ...
    reset(user: UserModel): void {
        user.totalCumulative += user.cumulative;
        user.cumulative = 0;
    }

    async resetAllUsers(): Promise<void> {
        const users = await this.UserRepository.getAllUsers({ hasCumulative: true });
